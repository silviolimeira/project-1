package com.sl.openjfx.business;
...
public class CalculaHorarioDeTrabalho {
	...
	public boolean testaIntersecaoPeriodos(Periodo periodo, ObservableList<Periodo> periodos) {
		...
		try {
			...
			for (int i = 0; i < max; i++) {
				...
				if (pte > pts) {
					if (pts > 0) {
						pt = new Periodo("00:00", String.format("%02d:%02d", pts / 60, pts % 60));
						if (intersecao(periodo, pt) > 0)
							return false;

					}
					pt = new Periodo(String.format("%02d:%02d", pte / 60, pte % 60), "23:59");
					if (intersecao(periodo, pt) > 0)
						return false;
				}
				else {
	...
	public void calculaHoraExtraAtraso(WorkScheduleReport horaExtra, WorkScheduleReport atraso,
									   WorkSchedule horarioDeTrabalho, WorkSchedule marcacoes) {
		...
		try {
			...
			for (int j = 0; j < maxMarcacoes; j++) {
				...
				calcMarcacoes(horarioDeTrabalho.getTable().getItems(), marcacoes.getTable().getItems(),
						atraso.getTable().getItems(), horaExtra.getTable().getItems());
	...
	public static void calculaHoraExtra(Periodo horarioTrabalho, List<Periodo> marcacoes, List<Periodo> horasExtras) {
		...
		for (int i = 0; i < max; i++) {
			...
			if ((inters & 0x01) == 0x01) {
				// System.out.println("### Marcacao: " + marcacao.toString() + "
				// ..............." + " horarioTrabalho: " + horarioTrabalho.toString());
				if (me > he && me < hs) {
					e = me;
					String mas_s = String.format("%02d:%02d", mas / 60, mas % 60);
					String he_s = String.format("%02d:%02d", he / 60, he % 60);
					if (mas > 0 && mas > he && mas > me)
						e = mas;
					if (ms < hs) {
	...
	public static void verificaPeriodosHE(List<Periodo> periodosHoraExtra, List<Periodo> marcacoes,
			List<Periodo> horasExtras, List<Periodo> horarioTrabalhoNorm) {
		Periodo m = null;
		Periodo phe = null;
	...
	public static void calcMarcacoes(List<Periodo> horarioDeTrabalho, List<Periodo> marcacoes, List<Periodo> atrasos,
			List<Periodo> horasExtras) {
